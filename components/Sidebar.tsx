...
export default function Sidebar({ user, onLogout, onAI }: { user: string; onLogout: () => void; onAI: () => void }) {
  ...
  const [collapsed, setCollapsed] = useState<Record<string, boolean>>({ Management: true, Chatter: true });
  ...
  return (
    <aside className="w-[240px] min-h-screen bg-[#161311] border-r border-white/[0.06] fixed top-0 left-0 z-50 flex flex-col">
      ...
      {/* AI Button - always visible at bottom */}
      <div className="px-2 py-2 border-t border-white/[0.06] shrink-0">
        <button onClick={onAI} className="w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-[12.5px] font-semibold text-[#4ade80] bg-[#4ade80]/[0.07] hover:bg-[#4ade80]/[0.12] transition border border-[#4ade80]/10">
          <I d={ICON.ai} />
          AI Assistant
          <div className="ml-auto w-1.5 h-1.5 rounded-full bg-[#4ade80] animate-pulse-slow" />
        </button>
      </div>

      {/* User */}
      <div className="px-3 py-3 border-t border-white/[0.06] flex items-center gap-2.5 shrink-0">
