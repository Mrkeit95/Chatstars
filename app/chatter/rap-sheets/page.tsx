...
export default function Page() {
  ...
  useEffect(() => {
    getBoards().then(b => {
      const all = b || [];
      setBoards(all);
      const match = all.find((board: any) => board.name.toLowerCase().includes("rap sheets"));
  ...
  if (!boardId) return (
    <div>
      <div className="mb-7"><h1 className="text-[26px] font-extrabold tracking-tight">Rap Sheets</h1><p className="text-[13px] text-[#78716c] mt-1">Performance records and evaluations</p></div>
  ...
  return <MondayBoard boardId={boardId} title="Rap Sheets" description="Performance records and evaluations" />;
}
