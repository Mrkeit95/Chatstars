...
export default function Page() {
  ...
  useEffect(() => {
    getBoards().then(b => {
      const all = b || [];
      setBoards(all);
      const match = all.find((board: any) => board.name.toLowerCase().includes("meetings"));
  ...
  if (!boardId) return (
    <div>
      <div className="mb-7"><h1 className="text-[26px] font-extrabold tracking-tight">Chatter Meetings</h1><p className="text-[13px] text-[#78716c] mt-1">Meeting notes and action items</p></div>
  ...
  return <MondayBoard boardId={boardId} title="Chatter Meetings" description="Meeting notes and action items" />;
}
