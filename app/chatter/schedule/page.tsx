...
export default function Page() {
  ...
  useEffect(() => {
    getBoards().then(b => {
      const all = b || [];
      setBoards(all);
      const match = all.find((board: any) => board.name.toLowerCase().includes("schedule"));
  ...
  if (!boardId) return (
    <div>
      <div className="mb-7"><h1 className="text-[26px] font-extrabold tracking-tight">Chatter Schedule</h1><p className="text-[13px] text-[#78716c] mt-1">Shift schedules and availability</p></div>
  ...
  return <MondayBoard boardId={boardId} title="Chatter Schedule" description="Shift schedules and availability" />;
}
