...
export default function Page() {
  ...
  useEffect(() => {
    getBoards().then(b => {
      const all = b || [];
      setBoards(all);
      const match = all.find((board: any) => board.name.toLowerCase().includes("feedback"));
  ...
  if (!boardId) return (
    <div>
      <div className="mb-7"><h1 className="text-[26px] font-extrabold tracking-tight">Anonymous Feedback</h1><p className="text-[13px] text-[#78716c] mt-1">Team feedback and suggestions</p></div>
  ...
  return <MondayBoard boardId={boardId} title="Anonymous Feedback" description="Team feedback and suggestions" />;
}
