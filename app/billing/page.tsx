...
import { fetchCreators, getActiveCreators, getAgencies, fmt, fmtFull, pct, colorFor, getCurrentMonth, getMonthLabel } from "@/lib/data";
...
export default function BillingPage() {
  ...
  const all = D; // Include everyone for company totals
  const active = getActiveCreators(D);
  const tR = all.reduce((a,c) => a+c.run, 0);
  const tTip = all.reduce((a,c) => a+c.tip, 0);
  const tSub = all.reduce((a,c) => a+c.ts, 0);
  const tFeb = all.reduce((a,c) => a+c.feb, 0);
  const tJan = all.reduce((a,c) => a+c.jan, 0);
  const tDec = all.reduce((a,c) => a+c.dec, 0);
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-4 gap-3.5 mb-7">
        <div className="p-5 rounded-xl bg-[#1a1714] border border-white/[0.06]"><div className="text-[10.5px] font-semibold text-[#78716c] uppercase tracking-wider mb-2.5">{SHORT[curIdx]} Revenue</div><div className="text-[28px] font-extrabold text-[#4ade80] leading-none">{fmtFull(tR)}</div></div>
        ...
        <div className="p-5 rounded-xl bg-[#1a1714] border border-white/[0.06]"><div className="text-[10.5px] font-semibold text-[#78716c] uppercase tracking-wider mb-2.5">Avg per Creator</div><div className="text-[28px] font-extrabold leading-none">{fmt(active.length > 0 ? tR / active.length : 0)}</div></div>
      </div>
      <div className="grid grid-cols-[1fr_1fr] gap-5 mb-7">
        ...
        <div className="bg-[#1a1714] border border-white/[0.06] rounded-xl p-6">
          ...
          <div className="grid grid-cols-3 gap-3 text-center">
            {[[SHORT[curIdx-2]||"Prev3", tJan, "#22d3ee"], [SHORT[curIdx-1]||"Prev2", tFeb, "#60a5fa"], [SHORT[curIdx], tR, "#4ade80"]].map(([label, val, color]) => (
          ...
          <div className="mt-3 p-4 bg-[#0c0a09] rounded-lg text-center">
            <div className="text-[9px] text-[#57534e] uppercase font-semibold mb-1.5">{SHORT[curIdx-3]||"Prev4"}</div>
