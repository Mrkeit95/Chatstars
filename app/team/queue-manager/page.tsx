...
export default function Page() {
  ...
  useEffect(() => {
    getBoards().then(b => {
      ...
      const match = all.find((board: any) => board.name.toLowerCase().includes("queue"));
  ...
  if (!boardId) return (
    <div>
      <div className="mb-7"><h1 className="text-[26px] font-extrabold tracking-tight">Queue Manager</h1><p className="text-[13px] text-[#78716c] mt-1">Queue management and workflow tracking</p></div>
  ...
  return <MondayBoard boardId={boardId} title="Queue Manager" description="Queue management and workflow tracking" accentColor="#22d3ee" />;
}
