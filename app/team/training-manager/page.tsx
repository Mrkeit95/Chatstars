...
export default function Page() {
  ...
  useEffect(() => {
    getBoards().then(b => {
      const all = b || [];
      setBoards(all);
      // Auto-match board by name
      const match = all.find((board: any) => board.name.toLowerCase().includes("training"));
  ...
  if (!boardId) return (
    <div>
      <div className="mb-7"><h1 className="text-[26px] font-extrabold tracking-tight">Training Manager</h1><p className="text-[13px] text-[#78716c] mt-1">Training programs, schedules, and progress</p></div>
      <div className="bg-[#1a1714] border border-white/[0.06] rounded-xl p-6">
        <p className="text-sm text-[#78716c] mb-4">Select the Monday.com board for this section:</p>
  ...
  return <MondayBoard boardId={boardId} title="Training Manager" description="Training programs, schedules, and progress" accentColor="#a78bfa" />;
}
