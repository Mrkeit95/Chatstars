...
export default function Page() {
  ...
  useEffect(() => {
    getBoards().then(b => {
      ...
      const match = all.find((board: any) => board.name.toLowerCase().includes("hiring"));
  ...
  if (!boardId) return (
    <div>
      <div className="mb-7"><h1 className="text-[26px] font-extrabold tracking-tight">Hiring Manager</h1><p className="text-[13px] text-[#78716c] mt-1">Recruitment pipeline and candidate tracking</p></div>
  ...
  return <MondayBoard boardId={boardId} title="Hiring Manager" description="Recruitment pipeline and candidate tracking" accentColor="#fb923c" />;
}
